...
struct thread_info *start_threads(struct options opt, struct nums *nums, struct thread_info *ej4)
{
    ...
    threads5 = malloc(sizeof(struct thread_info) * opt.num_threads);
    ...
    if (threads == NULL || ej4 == NULL || threads5 == NULL) {
    ...
    for (i = 0; i < opt.num_threads; i++) {
        threads5[i].args = malloc(sizeof(struct args));
        threads5[i].args->mutex_ej1 = mutex_ej1;  //EJERCICIO 1
        threads5[i].args->thread_num = i;
        threads5[i].args->nums       = nums;
        threads5[i].args->iterations = opt.iterations;

        if (0 != pthread_create(&threads5[i].id, NULL, decrement_ej5, threads5[i].args)) {
            printf("Could not create thread #%d", i);
            exit(1);
        }
    }
